...
-   **Listening Section Practice**: Per-section practice tests (Sections 1–4, 5 tests each, 20 total) with pre-generated MP3s at `artifacts/flashcards/public/audio/listening/<testId>-<idx>.mp3`. The `AudioPlayer` in `artifacts/flashcards/src/components/listening-skills.tsx` fetches all per-line clips in parallel and concatenates them into a single combined `Blob` (MP3 frames self-sync, so naive concatenation plays as one continuous stream in modern browsers). A shared in-flight build promise + generation token prevents preload/play races from leaking object URLs or overwriting each other.
